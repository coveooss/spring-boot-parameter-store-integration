...
If you still want it to run twice or if you are using [spring-boot-devtools](https://docs.spring.io/spring-boot/docs/current/reference/html/using-spring-boot.html#using-boot-devtools-restart) you can set the optional property `awsParameterStorePropertySource.disableMultipleContextSupport` to `false` The default for this property is `true`and therefore prevents a multiple initialization. If you are also using Spring Cloud, this property will only work if it is set in the bootstrap properties.
